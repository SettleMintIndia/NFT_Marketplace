...
task('placeholder', 'Sets up the metadata and image for the pre-reveal stage')
  ...
  .setAction(async ({ ipfsnode }: { ipfsnode: string }, hre) => {
    ...
    console.log(`Preparing ${chalk.yellow.bold(images.length)} placeholders for the pre-reveal stage:`);
    ...
    const metadata = JSON.parse(readFileSync('./assets/placeholder/placeholder.json', 'utf8'));
    metadata.image = `ipfs://${imageCID}`;
    mkdirSync('./assets/generated/', { recursive: true });
    writeFileSync('./assets/generated/placeholder.json', JSON.stringify(metadata, null, 2));
