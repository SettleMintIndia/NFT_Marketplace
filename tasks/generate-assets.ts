...
task('generate-assets', 'Generates everything needed to reveal')
  .addParam<number>('common', 'the amount of commons to generate', 10, types.int)
  .addParam<number>('limited', 'the amount of limited to generate', 5, types.int)
  ...
  .setAction(
    async (
      ...
      for (const { subject, rarity, cardindex, totalcards } of randomized) {
        ...
        const metadata = JSON.parse(readFileSync(`./assets/metadata/${subject}.json`, 'utf8'));
        ...
        writeFileSync(`./assets/generated/cards/${index}.json`, JSON.stringify(finalMetadata, null, 2));
...
task('generate-asset', 'Generates a card image')
  .addParam<string>('subject', 'the name of the subject as used in the filename in the subjects folder')
  .addParam<string>('rarity', 'the rarity of the card')
  .addParam<number>('cardindex', 'index for this card for this rarity', 1, types.int)
  .addParam<number>('totalcards', 'amount of cards per dog for this rarity', 10, types.int)
  ...
  .setAction(
    async (
      ...
      console.log(`Created edition: ${tokenId}, with DNA: ${subject} @ ${rarity} (${cardindex}/${totalcards})`);
      ...
      const backgroundDimensions = sizeOf(join('./assets/layers/backgrounds', `${rarity}.png`));
      ...
      // setup the canvas
      const artMax = 700;
      const isWide = backgroundDimensions.height / backgroundDimensions.width < 1;
      const width = Math.round(isWide ? artMax : (backgroundDimensions.width / backgroundDimensions.height) * artMax);
      const height = Math.round(isWide ? (backgroundDimensions.height / backgroundDimensions.width) * artMax : artMax);
      ...
      // draw background
      const backgroundImage = await loadImage(join('./assets/layers/backgrounds', `${rarity}.png`));
      ctx.drawImage(
        backgroundImage,
        0,
        0,
      ...
      const subjectDimensions = sizeOf(join('./assets/layers/subjects', `${subject}.png`));
      if (!subjectDimensions.width || !subjectDimensions.height) {
        throw new Error('Could not get the dimensions of the image');
      }

      // draw subject
      const subjectImage = await loadImage(join('./assets/layers/subjects', `${subject}.png`));
      ctx.drawImage(subjectImage, 0, 0, subjectDimensions.width, subjectDimensions.height, 0, 0, width, height);

      // get the overlay image dimensions
      const overlayDimensions = sizeOf(join('./assets/layers/overlays', `${rarity}.png`));
      if (!overlayDimensions.width || !overlayDimensions.height) {
        throw new Error('Could not get the dimensions of the image');
      }

      // draw overlay
      const overlayImage = await loadImage(join('./assets/layers/overlays', `${rarity}.png`));
      ctx.drawImage(overlayImage, 0, 0, overlayDimensions.width, overlayDimensions.height, 0, 0, width, height);
      ...
      ctx.fillText(
        metadata.attributes.find((attr: { trait_type: string; value: string }) => attr.trait_type === 'Breed')?.value,
        width - 20,
        30
      );
      ...
      ctx.fillText(
        `${
          metadata.attributes.find((attr: { trait_type: string; value: string }) => attr.trait_type === 'Shedding')
            ?.value
        }/5`,
        60,
        630
      ...
      ctx.fillText(
        `${
          metadata.attributes.find((attr: { trait_type: string; value: string }) => attr.trait_type === 'Affectionate')
            ?.value
        }/5`,
        Math.round(width / 2),
        630
      ...
      ctx.fillText(
        `${
          metadata.attributes.find((attr: { trait_type: string; value: string }) => attr.trait_type === 'Playfulness')
            ?.value
        }/5`,
        360,
        630
      ...
      // save image
      await sharp(canvas.toBuffer('image/png', { compressionLevel: 0, filters: canvas.PNG_FILTER_NONE }))
