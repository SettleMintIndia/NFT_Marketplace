...
import { ItemListed, RegisterCollection } from '../../generated/marketplace/Marketplace';
import { Collection, NftTransaction } from '../../generated/schema';
...
export function handleItemListed(event: ItemListed): void {
  ...
  const try_tokenURI = erc721.try_tokenURI(BigInt.fromI32(1));

  let transaction = NftTransaction.load(transactionHash);

  if (transaction === null) {
    transaction = new NftTransaction(transactionHash);
  }

  // transaction.uri = uri;
  transaction.uri = try_tokenURI.reverted ? '' : try_tokenURI.value;
  transaction.transactionType = 'List';
